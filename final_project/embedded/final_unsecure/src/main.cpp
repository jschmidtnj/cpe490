...
struct Message {
  ...
  byte type;
  int hash;
  Message(std::string _payload, byte _destination, byte _sender, byte _id,
          byte _frame, byte _packet, byte _packetLocation, byte _type)
      : payload{_payload},
...
void printStations() {
  ...
  for (int i = 0; i < stationList.num; i++) {
    ...
    if (debug_mode) DBG_OUTPUT_PORT.println(mac);
    char macChar[25];
...
void onReceive(int packetSize) {
  ...
  Message message(payload, recipient, sender, id, frame, packet, packetLocation,
                  type);
  if (recipient != address) {
    if (debug_mode)
      DBG_OUTPUT_PORT.println("this message is not for me. It's for " +
                              String(recipient));
  ...
  if (message.packetLocation == endType ||
      message.packetLocation == startEndType) {
    ...
    std::string key =
        message.type == typeMessage
            ? "message"
            : (message.type == typeLocation ? "location" : "unknown");
...
void handleNotFound(AsyncWebServerRequest *request) {
  ...
  if (debug_mode) DBG_OUTPUT_PORT.print(message);
}
...
void saveMessages(std::string fullmessage, byte destination, byte id,
                  byte frame, byte packetLocation, byte type) {
  // split into multiple packets
  int endCount = ceil((double)fullmessage.length() / maxLoraPacket);
  for (int i = 0; i < endCount; i++) {
    byte location = midType;
    if (i == 0 || i == endCount - 1) location = packetLocation;
    outgoingMessages.push(
        Message(fullmessage.substr(i * maxLoraPacket, maxLoraPacket),
                destination, address, id, frame, i, location, type));
  }
}

void handleError(AsyncWebServerRequest *request, int errorCode,
                 String errorMessage) {
  ...
  if (type == WS_EVT_CONNECT) {
    ...
    if (info->final && info->index == 0 && info->len == len) {
      ...
      if (info->opcode == WS_TEXT) {
        destination = data[0];
        type = data[1];
        for (size_t i = 2; i < info->len; i++) msg += (char)data[i];
      } else {
        if (debug_mode && verbose)
      ...
      if (debug_mode && verbose)
        DBG_OUTPUT_PORT.printf(
            "ws[%s][%u] frame[%u] %s[%llu - %llu]: ", server->url(),
            client->id(), info->num,
      ...
      if (info->opcode == WS_TEXT) {
        for (size_t i = startIndex; i < len; i++) msg += (char)data[i];
      } else {
        if (debug_mode && verbose)
          DBG_OUTPUT_PORT.println("received binary data");
        return;
      ...
      if (debug_mode && verbose) DBG_OUTPUT_PORT.printf("%s\n", msg.c_str());
      ...
      if (frameType == endType &&
          messageHeaders.find(client) != messageHeaders.end()) {
...
void setup() {
  ...
  WiFi.mode(WIFI_AP);
  if (debug_mode) DBG_OUTPUT_PORT.println("#Wait 100 ms for AP_START...");
  delay(100);
  WiFi.softAP(ssid, password, channel_num, 0, max_connection);
  if (debug_mode) DBG_OUTPUT_PORT.println("#Set softAPConfig");
  ...
  server.on("/senders", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    for (const byte &sender : pastSenders) {
      if (debug_mode && verbose) DBG_OUTPUT_PORT.println(sender);
  ...
  server.on("/location", HTTP_PUT, [](AsyncWebServerRequest *request) {}, NULL,
            [](AsyncWebServerRequest *request, uint8_t *data, size_t len,
              ...
              String message = "";
              if (dataObj.containsKey("longitude") &&
                  dataObj.containsKey("longitude")) {
                if (!dataObj.containsKey("longitude")) {
                  handleError(request, 500, "no longitude key found");
                  return;
                }
                if (!dataObj.containsKey("latitude")) {
                  handleError(request, 500, "no latitude key found");
                  return;
                }
                String latitude = dataObj["latitude"];
                String longitude = dataObj["longitude"];
                message = latitude + "," + longitude;
              }
              byte messageDestination = dataObj["destination"];
              saveMessages(message.c_str(), messageDestination, msgCount, 0,
                           startEndType, typeLocation);
              DynamicJsonDocument successObj(200);
              successObj["message"] = "sent location";
              String successStr;
              serializeJson(successObj, successStr);
  ...
  if (debug_mode) DBG_OUTPUT_PORT.println("#finished creating server");
  server.begin();
  if (debug_mode) DBG_OUTPUT_PORT.println("#HTTP server started");
...
void loop() {
  if (create_local_host) dnsServer.processNextRequest();
